...
# JWT token configuration - will be set when app is initialized
JWT_SECRET = None
JWT_EXPIRATION_HOURS = 24

def init_mobile_api(app, db, models):
    """Initialize mobile API with app context and models"""
    global JWT_SECRET
    JWT_SECRET = app.config['SECRET_KEY']
    
    # Store references to models and db
    api.db = db
    api.User = models['User']
    api.ContractorProfile = models['ContractorProfile'] 
    api.CustomerProfile = models['CustomerProfile']
    api.WorkRequest = models['WorkRequest']
    api.ContractorInvoice = models['ContractorInvoice']
    api.SERVICE_CATEGORIES = models['SERVICE_CATEGORIES']
    api.GEOGRAPHIC_AREAS = models['GEOGRAPHIC_AREAS']
    api.validate_password = models['validate_password']
...
def mobile_auth_required(f):
    ...
    def decorated_function(*args, **kwargs):
        ...
        kwargs['current_mobile_user'] = user
...
def mobile_login():
    ...
    user = api.User.query.filter_by(email=data['email']).first()
...
def mobile_register():
    ...
    if api.User.query.filter_by(email=data['email']).first():
        return jsonify({'error': 'Email already registered'}), 400
    
    # Validate password
    is_valid, message = api.validate_password(data['password'])
    if not is_valid:
...
def get_user_profile(current_mobile_user):
...
@mobile_auth_required
def update_user_profile(current_mobile_user):
    """Update user profile"""
    user = current_mobile_user
    data = request.get_json()
    ...
    if user.account_type == 'contractor':
        profile = user.contractor_profile
        if not profile:
            profile = api.ContractorProfile(user_id=user.id)
            api.db.session.add(profile)
        ...
        profile = user.customer_profile
        if not profile:
            profile = api.CustomerProfile(user_id=user.id)
            api.db.session.add(profile)
...
@mobile_auth_required
def get_work_requests(current_mobile_user):
    """Get work requests for current user"""
    user = current_mobile_user
    
    if user.account_type == 'contractor':
        requests = api.WorkRequest.query.filter_by(contractor_id=user.id).all()
    elif user.account_type == 'customer':
        requests = api.WorkRequest.query.filter_by(customer_id=user.id).all()
    else:
...
@mobile_auth_required
def create_work_request(current_mobile_user):
    ...
    api.db.session.add(work_request)
    api.db.session.commit()
...
@mobile_auth_required
def update_work_request(current_mobile_user, request_id):
    """Update work request status"""
    user = current_mobile_user
    data = request.get_json()
    
    work_request = api.WorkRequest.query.get_or_404(request_id)
...
@mobile_auth_required
def search_contractors(current_mobile_user):
    """Search for contractors based on criteria"""
    data = request.get_json()
    
    query = api.db.session.query(api.ContractorProfile).join(api.User)
    
    # Filter by service categories
    if data.get('service_categories'):
        services = data['service_categories']
        for service in services:
            query = query.filter(api.ContractorProfile.services.contains(service))
    
    # Filter by geographic area
    if data.get('geographic_area'):
        query = query.filter(api.ContractorProfile.geographic_area == data['geographic_area'])
    ...
    if data.get('location') and data.get('max_distance'):
        ...
        query = query.filter(api.ContractorProfile.service_radius >= data['max_distance'])
...
def get_app_config():
    ...
    return jsonify({
        ...
        'geographic_areas': api.GEOGRAPHIC_AREAS,
        'version': '1.0.0',
...
@mobile_auth_required
def get_invoices(current_mobile_user):
    """Get invoices for current user"""
    user = current_mobile_user
    
    if user.account_type == 'contractor':
        invoices = api.ContractorInvoice.query.filter_by(contractor_id=user.id).all()
    elif user.account_type == 'customer':
        invoices = api.ContractorInvoice.query.filter_by(customer_id=user.id).all()
    else:
        return jsonify({'error': 'Unauthorized'}), 403
...
@mobile_auth_required
def get_user_stats(current_mobile_user):
    """Get user statistics"""
    user = current_mobile_user
    
    if user.account_type == 'contractor':
        total_requests = api.WorkRequest.query.filter_by(contractor_id=user.id).count()
        completed_requests = api.WorkRequest.query.filter_by(
            contractor_id=user.id, 
            status='completed'
        ).count()
        pending_requests = api.WorkRequest.query.filter_by(
            contractor_id=user.id, 
            status='pending'
        ).count()
        ...
        total_requests = api.WorkRequest.query.filter_by(customer_id=user.id).count()
        completed_requests = api.WorkRequest.query.filter_by(
            customer_id=user.id, 
            status='completed'
        ).count()
